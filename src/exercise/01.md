...
- This is useful when we have complicated logic for state changes, like in cases
  when we pass in output of `useReducer()` inside a Context Provider.
- Instead of keeping the state change logic for a particular function in the
  child component, we make a function which contains that logic in the file
  where we create our context.
- So, when our components need to update the state, they use specific functions
  for that use case instead of passing string action types.
- This is also useful for code splitting and tree-shaking.
